...
docker exec $ESCONT curl -s -XPUT "http://localhost:9200/_snapshot/esbackup" -H "Content-Type: application/json;charset=UTF-8" -d '{"type": "fs","settings": {"location": "/backups"}}' > /dev/null

# Delete old snapshot so that the new snapshot is not incremental
docker exec $ESCONT curl -s -XDELETE "localhost:9200/_snapshot/esbackup/snapshot_1" > /dev/null
